...
class Text:
    _target_strings: dict[TextTarget, str]
    ...
    def subscript(self, subscript: Any):
        return self + Text("_", latex="_{") + text(subscript) + Text(latex="}")
    ...
    def matches(self, query: Any, *, try_text=True, case_sensitive=False):
        if query == self:
            return True
        if query in self._target_strings.values():
            return True
    ...
    def join(self, parts: Iterable[Any]):
